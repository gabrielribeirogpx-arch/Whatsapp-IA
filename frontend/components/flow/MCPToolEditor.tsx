...
export default function MCPToolEditor({ draft, tools, onDraftChange }: Props) {
  ...
  return <div className="mcp-editor" aria-label="Configuração da ferramenta MCP">
    ...
    <section className="mcp-config-card" aria-labelledby="mcp-connection-title">
      ...
      <label className="mcp-field"><span>Selecionar conexão</span><select aria-label="Selecionar conexão MCP" value={connectionId} onChange={(event) => { setTestState('idle'); setLatency(null); onDraftChange({ connection_id: event.target.value, server_name: servers.find(([id]) => id === event.target.value)?.[1] || '', tool_name: '', arguments: {} }); }}><option value="">Selecione uma conexão</option>{servers.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></label>
      <div className="mcp-button-row"><Link className="mcp-secondary-button" href="/dashboard/ai/mcp" target="_blank" aria-label="Criar nova conexão MCP em outra aba">+ Nova conexão <ExternalLink size={13} /></Link><button className="mcp-test-button" type="button" disabled={!connectionId || testState === 'testing'} onClick={testConnection}>{testState === 'testing' ? <><span className="mcp-spinner" /> Testando...</> : 'Testar conexão'}</button></div>
