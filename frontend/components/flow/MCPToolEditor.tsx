...
export default function MCPToolEditor({ draft, tools, onDraftChange }: Props) {
  ...
  return <div className="mcp-editor" aria-label="Configuração da ferramenta MCP">
    ...
    <section className="mcp-config-card" aria-labelledby="mcp-tool-title">
      ...
      {['WRITE', 'DESTRUCTIVE', 'DELETE'].includes(classification) ? <div className="mcp-write-authorization"><label className="flow-editor-checkbox"><input aria-label="Permitir alteração em sistema externo" type="checkbox" checked={draft.allow_external_write === true} onChange={(event) => onDraftChange({ allow_external_write: event.target.checked })} /><span><strong>Permitir alteração em sistema externo</strong><small>Esta ferramenta pode criar ou alterar dados em um sistema externo.</small></span></label>{['DESTRUCTIVE', 'DELETE'].includes(classification) ? <label className="flow-editor-checkbox"><input type="checkbox" checked={draft.destructive_confirmed === true} onChange={(event) => onDraftChange({ destructive_confirmed: event.target.checked })} /><span><strong>Confirmar ação destrutiva</strong><small>Esta autorização é adicional à permissão de alteração externa.</small></span></label> : null}</div> : null}
    </section>
    ...
    <section className="mcp-config-card" aria-labelledby="mcp-execution-title">
      ...
      <details className="mcp-advanced-result"><summary>Opções avançadas de execução</summary><label className="mcp-field"><span>Idempotency key</span><input value={text(draft.idempotency_key)} onChange={(event) => onDraftChange({ idempotency_key: event.target.value })} placeholder="{{session.id}}:{{selected_slot_id}}" /></label></details>
    </section>
  </div>;
}
