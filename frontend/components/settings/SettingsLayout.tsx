    ...
    <section className='w-full min-w-0 px-4 py-6 sm:px-6 lg:px-8'>
      <div className='mx-auto flex w-full max-w-7xl flex-col gap-5'>
        <header className='relative overflow-hidden rounded-3xl border border-[color:var(--surface-border)] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-6 text-white shadow-[0_24px_60px_-38px_rgba(2,6,23,0.9)] md:p-8'>
          ...
          <p className='inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold text-emerald-100 shadow-sm backdrop-blur'>WhatsApp Business Console</p>
          <div className='mt-4 flex flex-wrap items-center gap-3'>
            <h1 className='text-2xl font-semibold tracking-tight md:text-[1.9rem]'>Configurações do WhatsApp</h1>
            <span className='rounded-full border border-emerald-300/30 bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-100'>Operational Console</span>
          </div>
          <p className='mt-2 max-w-3xl text-sm leading-relaxed text-slate-300'>Gerencie API Keys, providers, templates e conexões Meta em um console dedicado ao WhatsApp Business.</p>
        </header>
