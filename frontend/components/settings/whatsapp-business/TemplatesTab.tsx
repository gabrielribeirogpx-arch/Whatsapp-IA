...
export default function TemplatesTab(props: any) {
  const { templates, providers = [] } = props;
  return <div className='w-full min-w-0 space-y-4'>
    ...
    {props.loading && templates.length === 0 ? <div className='grid w-full min-w-0 gap-3 md:grid-cols-2'>{Array.from({ length: 2 }).map((_, i) => <div key={i} className='h-40 animate-pulse rounded-2xl border border-slate-200 bg-gradient-to-r from-slate-100 via-slate-50 to-slate-100' />)}</div> : templates.length === 0 ? <div className='settings-card rounded-2xl border border-dashed border-slate-300 bg-white/70 p-10 text-center text-slate-600'><FileText className='mx-auto mb-3 text-slate-400' size={24} />Crie templates aprováveis para mensagens fora da janela de 24 horas.</div> : <div className='grid w-full min-w-0 gap-3'>{templates.map((t: any) => { const provider = providers.find((p: any) => p.id === t.provider_id); return <TemplateCard key={t.id} t={t} providerName={provider?.display_name || provider?.provider_type} providerStatus={provider?.connection_status || provider?.status} onSubmit={() => props.onSubmitTemplate(t.id)} loading={props.loading} />; })}</div>}
  </div>;
}
