...
export default function TemplateCard({ t, onSubmit, loading, providerName, providerStatus }: { t: any; onSubmit: any; loading: boolean; providerName?: string; providerStatus?: string }) {
  return <article className='w-full min-w-0 rounded-2xl border border-[color:var(--surface-border)] bg-white/95 p-4 shadow-[0_16px_30px_-28px_rgba(15,23,42,0.8)] transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_24px_40px_-30px_rgba(15,23,42,0.75)]'>
    ...
    <div className='mt-3 flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500'>
      ...
      <span className='inline-flex items-center gap-2'>Provider associado: <span className='rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5'>{providerName || 'N/D'}</span>{providerStatus && <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${providerStatus === 'connected' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>{providerStatus}</span>}</span>
    </div>
