...
import { AlertCircle, Bell, Building2, CalendarDays, CheckCircle2, FolderOpen, Clock3, CreditCard, Globe2, KeyRound, Layers3, Loader2, LockKeyhole, LogOut, Mail, MessageSquareText, Pencil, Plus, Save, ShieldCheck, Smartphone, User, UsersRound, XCircle } from 'lucide-react';
...
function IntegrationsTab() {
  ...
  const [driveStatus, setDriveStatus] = useState<GoogleCalendarConnectionStatus | null>(null);
  const [loadingDrive, setLoadingDrive] = useState(true);
  const [driveActionLoading, setDriveActionLoading] = useState(false);
  const [driveMessage, setDriveMessage] = useState('');
  const [driveError, setDriveError] = useState('');
  const [sheetsStatus, setSheetsStatus] = useState<GoogleCalendarConnectionStatus | null>(null);
  ...
  useEffect(() => {
    ...
    if (isGoogleDriveReturn && status === 'connected') refreshDriveStatus('Google Drive conectado com sucesso.');
    else if (isGoogleDriveReturn && status === 'error') { refreshDriveStatus(); setDriveError('Falha ao conectar Google Drive.'); setDriveMessage(''); }
    else refreshDriveStatus();
    if (isGoogleSheetsReturn && status === 'connected') refreshSheetsStatus('Google Sheets conectado com sucesso.');
    else if (isGoogleSheetsReturn && status === 'error') { refreshSheetsStatus(); setSheetsError('Falha ao conectar Google Sheets.'); setSheetsMessage(''); }
    else refreshSheetsStatus();
  ...
  const refreshDriveStatus = async (successMessage?: string) => {
    ...
    try { const status = await getGoogleDriveStatus(); setDriveStatus(status); setDriveMessage(successMessage || ''); }
    catch { setDriveError('Falha ao carregar status do Google Drive.'); setDriveMessage(''); }
    finally { setLoadingDrive(false); }
  };

  const connectDrive = () => { setDriveError(''); try { window.location.href = getGoogleDriveConnectUrl(); } catch { setDriveError('Falha ao conectar Google Drive.'); } };
  const disconnectDrive = async () => { setDriveActionLoading(true); setDriveError(''); try { const status = await disconnectGoogleDrive(); setDriveStatus(status); setDriveMessage('Não conectado'); } catch { setDriveError('Falha ao desconectar Google Drive'); } finally { setDriveActionLoading(false); } };
  const refreshSheetsStatus = async (successMessage?: string) => {
    setLoadingSheets(true); setSheetsError('');
    try { const status = await getGoogleSheetsStatus(); setSheetsStatus(status); setSheetsMessage(successMessage || ''); }
    catch { setSheetsError('Falha ao carregar status do Google Sheets.'); setSheetsMessage(''); }
    finally { setLoadingSheets(false); }
  };
  const connectSheets = () => { setSheetsError(''); try { window.location.href = getGoogleSheetsConnectUrl(); } catch { setSheetsError('Falha ao conectar Google Sheets.'); } };
  const disconnectSheets = async () => { setSheetsActionLoading(true); setSheetsError(''); try { const status = await disconnectGoogleSheets(); setSheetsStatus(status); setSheetsMessage('Não conectado'); } catch { setSheetsError('Falha ao desconectar Google Sheets'); } finally { setSheetsActionLoading(false); } };
  ...
  const sheetsAccountEmail = typeof sheetsStatus?.metadata?.account_email === 'string' ? sheetsStatus.metadata.account_email : 'Nenhuma conta conectada';
  const driveAccountEmail = typeof driveStatus?.metadata?.account_email === 'string' ? driveStatus.metadata.account_email : 'Nenhuma conta conectada';
  ...
  return <Card><HubHeader icon={Layers3} eyebrow='Integration Catalog' title='Integrações' description='Visão executiva dos conectores essenciais do workspace com status visual.' />
    ...
    <div className='border-t border-slate-100 p-5 md:p-6'><div className='mb-4'><p className='text-xs font-semibold uppercase tracking-[0.14em] text-slate-500'>Apps conectados ao Wazza</p><h3 className='mt-1 text-lg font-semibold text-slate-950'>Conecte apps ao workspace</h3></div>
      <div className='rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-emerald-50/40 p-5 shadow-sm'><div className='flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between'><div className='flex gap-4'><span className='flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700'><CalendarDays size={22} /></span><div><div className='flex flex-wrap items-center gap-2'><h4 className='text-base font-semibold text-slate-950'>Google Calendar</h4><span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${isConnected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{loadingCalendar ? <Loader2 size={12} className='animate-spin' /> : isConnected ? <CheckCircle2 size={12} /> : <XCircle size={12} />}{loadingCalendar ? 'Carregando...' : isConnected ? 'Conectado' : 'Não conectado'}</span></div><p className='mt-2 text-sm text-slate-600'>Permita que a IA consulte disponibilidade e eventos sem expor tokens no frontend.</p><div className='mt-4 grid gap-2 text-sm sm:grid-cols-2'><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Provider</b>{calendarStatus?.provider || 'google_calendar'}</p><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Conta</b>{loadingCalendar ? 'Consultando status...' : accountEmail}</p></div></div></div><div className='flex w-full flex-col gap-2 sm:w-auto sm:min-w-56'>{isConnected ? <button type='button' disabled={calendarActionLoading || loadingCalendar} onClick={disconnectCalendar} className='inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-white px-5 py-3 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60'>{calendarActionLoading ? <Loader2 size={16} className='animate-spin' /> : <XCircle size={16} />} Desconectar</button> : <button type='button' disabled={loadingCalendar} onClick={connectCalendar} className='inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60'><CalendarDays size={16} /> Conectar Google Calendar</button>}<button type='button' disabled={loadingCalendar} onClick={() => refreshCalendarStatus()} className='rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60'>Atualizar status</button></div></div>{calendarMessage && <p className={`mt-4 inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold ${isConnected ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : 'border border-slate-200 bg-white text-slate-600'}`}><CheckCircle2 size={16} /> {calendarMessage}</p>}{calendarError && <p className='mt-4 inline-flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700'><AlertCircle size={16} /> {calendarError}</p>}</div>
      <div className='rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-blue-50/40 p-5 shadow-sm'><div className='flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between'><div className='flex gap-4'><span className='flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-blue-100 text-blue-700'><FolderOpen size={22} /></span><div><div className='flex flex-wrap items-center gap-2'><h4 className='text-base font-semibold text-slate-950'>Google Drive</h4><span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${isDriveConnected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{loadingDrive ? <Loader2 size={12} className='animate-spin' /> : isDriveConnected ? <CheckCircle2 size={12} /> : <XCircle size={12} />}{loadingDrive ? 'Carregando...' : isDriveConnected ? 'Conectado' : 'Não conectado'}</span></div><p className='mt-2 text-sm text-slate-600'>Permita que a IA liste, busque, leia e crie arquivos sem expor tokens no frontend.</p><div className='mt-4 grid gap-2 text-sm sm:grid-cols-2'><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Provider</b>{driveStatus?.provider || 'google_drive'}</p><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Conta</b>{loadingDrive ? 'Consultando status...' : driveAccountEmail}</p></div></div></div><div className='flex w-full flex-col gap-2 sm:w-auto sm:min-w-56'>{isDriveConnected ? <button type='button' disabled={driveActionLoading || loadingDrive} onClick={disconnectDrive} className='inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-white px-5 py-3 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60'>{driveActionLoading ? <Loader2 size={16} className='animate-spin' /> : <XCircle size={16} />} Desconectar</button> : <button type='button' disabled={loadingDrive} onClick={connectDrive} className='inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60'><FolderOpen size={16} /> Conectar Google Drive</button>}<button type='button' disabled={loadingDrive} onClick={() => refreshDriveStatus()} className='rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60'>Atualizar status</button></div></div>{driveMessage && <p className={`mt-4 inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold ${isDriveConnected ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : 'border border-slate-200 bg-white text-slate-600'}`}><CheckCircle2 size={16} /> {driveMessage}</p>}{driveError && <p className='mt-4 inline-flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700'><AlertCircle size={16} /> {driveError}</p>}</div>
      <div className='rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-green-50/40 p-5 shadow-sm'><div className='flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between'><div className='flex gap-4'><span className='flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-green-100 text-green-700'><FolderOpen size={22} /></span><div><div className='flex flex-wrap items-center gap-2'><h4 className='text-base font-semibold text-slate-950'>Google Sheets</h4><span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${isSheetsConnected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{loadingSheets ? <Loader2 size={12} className='animate-spin' /> : isSheetsConnected ? <CheckCircle2 size={12} /> : <XCircle size={12} />}{loadingSheets ? 'Carregando...' : isSheetsConnected ? 'Conectado' : 'Não conectado'}</span></div><p className='mt-2 text-sm text-slate-600'>Permita que a IA liste, leia, crie e atualize planilhas.</p><div className='mt-4 grid gap-2 text-sm sm:grid-cols-2'><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Provider</b>{sheetsStatus?.provider || 'google_sheets'}</p><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Conta</b>{loadingSheets ? 'Consultando status...' : sheetsAccountEmail}</p></div></div></div><div className='flex w-full flex-col gap-2 sm:w-auto sm:min-w-56'><button type='button' disabled={loadingSheets || isSheetsConnected} onClick={connectSheets} className='inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60'><FolderOpen size={16} /> Conectar Google Sheets</button><button type='button' disabled={sheetsActionLoading || loadingSheets || !isSheetsConnected} onClick={disconnectSheets} className='inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-white px-5 py-3 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60'>{sheetsActionLoading ? <Loader2 size={16} className='animate-spin' /> : <XCircle size={16} />} Desconectar</button><button type='button' disabled={loadingSheets} onClick={() => refreshSheetsStatus()} className='rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60'>Atualizar status</button></div></div><div className='mt-4 rounded-2xl bg-white/80 px-4 py-3 text-sm text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Ferramentas oficiais</b><ul className='mt-2 grid gap-1 font-mono text-xs text-slate-700 sm:grid-cols-2'><li>google_sheets_list_spreadsheets</li><li>google_sheets_read_sheet</li><li>google_sheets_append_row</li><li>google_sheets_update_row</li><li>google_sheets_create_spreadsheet</li></ul></div>{sheetsMessage && <p className={`mt-4 inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold ${isSheetsConnected ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : 'border border-slate-200 bg-white text-slate-600'}`}><CheckCircle2 size={16} /> {sheetsMessage}</p>}{sheetsError && <p className='mt-4 inline-flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700'><AlertCircle size={16} /> {sheetsError}</p>}</div></div>
  </Card>;
}
...
function WhatsAppBusinessConsole() {
  ...
  return <div className='w-full min-w-0 space-y-4'>
    ...
    {tab === 'connection' && <ProvidersTab providers={providers} form={providerForm} setForm={setProviderForm} loading={loading} onSubmit={(e: FormEvent) => run(async () => { e.preventDefault(); await createWhatsAppProvider(providerForm); setProviderForm(baseProviderForm); await refresh(); }, 'Conexão salva com sucesso', 'Falha ao salvar conexão')} onTest={(id: string) => run(async () => { await testWhatsAppProvider(id); await refresh(); }, 'Conexão testada com sucesso', 'Falha ao testar conexão')} onActivate={(id: string) => run(async () => { await activateWhatsAppProvider(id); await refresh(); }, 'Conexão ativada com sucesso', 'Falha ao ativar conexão')} onDelete={(id: string) => run(async () => { await deleteWhatsAppProvider(id); await refresh(); }, 'Conexão removida com sucesso', 'Falha ao remover conexão')} onEdit={(id: string, payload: Record<string, unknown>) => run(async () => { await updateWhatsAppProvider(id, payload); await testWhatsAppProvider(id); await refresh(); }, 'Conexão atualizada', 'Falha ao atualizar conexão')} onMetaConnected={refresh} />}
