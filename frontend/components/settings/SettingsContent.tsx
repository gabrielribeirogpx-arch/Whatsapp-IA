...
function IntegrationsTab() {
  ...
  return <Card><HubHeader icon={Layers3} eyebrow='Integration Catalog' title='Integrações' description='Visão executiva dos conectores essenciais do workspace com status visual.' />
    ...
    <div className='border-t border-slate-100 p-5 md:p-6'><div className='mb-4'><p className='text-xs font-semibold uppercase tracking-[0.14em] text-slate-500'>Integrações externas</p><h3 className='mt-1 text-lg font-semibold text-slate-950'>Conecte apps ao workspace</h3></div>
      ...
      <div className='rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-green-50/40 p-5 shadow-sm'><div className='flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between'><div className='flex gap-4'><span className='flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-green-100 text-green-700'><FolderOpen size={22} /></span><div><div className='flex flex-wrap items-center gap-2'><h4 className='text-base font-semibold text-slate-950'>Google Sheets</h4><span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${isSheetsConnected ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{loadingSheets ? <Loader2 size={12} className='animate-spin' /> : isSheetsConnected ? <CheckCircle2 size={12} /> : <XCircle size={12} />}{loadingSheets ? 'Carregando...' : isSheetsConnected ? 'Conectado' : 'Não conectado'}</span></div><p className='mt-2 text-sm text-slate-600'>Permita que a IA liste, leia, crie e atualize planilhas com confirmação para alterações.</p><div className='mt-4 grid gap-2 text-sm sm:grid-cols-2'><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Provider</b>{sheetsStatus?.provider || 'google_sheets'}</p><p className='rounded-2xl bg-white/80 px-4 py-3 text-slate-600'><b className='block text-xs uppercase tracking-[0.12em] text-slate-400'>Conta</b>{loadingSheets ? 'Consultando status...' : sheetsAccountEmail}</p></div></div></div><div className='flex w-full flex-col gap-2 sm:w-auto sm:min-w-56'>{isSheetsConnected ? <button type='button' disabled={sheetsActionLoading || loadingSheets} onClick={disconnectSheets} className='inline-flex items-center justify-center gap-2 rounded-2xl border border-red-200 bg-white px-5 py-3 text-sm font-semibold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-60'>{sheetsActionLoading ? <Loader2 size={16} className='animate-spin' /> : <XCircle size={16} />} Desconectar</button> : <button type='button' disabled={loadingSheets} onClick={connectSheets} className='inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60'><FolderOpen size={16} /> Conectar Google Sheets</button>}<button type='button' disabled={loadingSheets} onClick={() => refreshSheetsStatus()} className='rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60'>Atualizar status</button></div></div>{sheetsMessage && <p className={`mt-4 inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-semibold ${isSheetsConnected ? 'border border-emerald-200 bg-emerald-50 text-emerald-700' : 'border border-slate-200 bg-white text-slate-600'}`}><CheckCircle2 size={16} /> {sheetsMessage}</p>}{sheetsError && <p className='mt-4 inline-flex items-center gap-2 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700'><AlertCircle size={16} /> {sheetsError}</p>}</div></div>
  </Card>;
}
