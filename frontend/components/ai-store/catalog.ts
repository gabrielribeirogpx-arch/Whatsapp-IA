...
export const MARKETPLACE_CATALOG: readonly AIStoreCardData[] = [
  ...
  ...kitSegments.map((name) => ({ ...make(name, 'Kit de Negócio', 'Sistema Completo', name, ['Start','Message','Condition','AI Agent','CRM','Tags','Transfer to Human','End']), subtitle: `Operação completa e mensurável para ${name}, pronta para adaptar.`, businessKit: makeBusinessKit(name), installManifest: { flows: [{ methodologies: makeBusinessKit(name).methodologies.map((item) => ({ id: item.id, version: item.version })) }], ai_agents: ['Agente especializado opcional'], knowledge_bases: makeBusinessKit(name).knowledgeBase, pipelines: makeBusinessKit(name).pipeline, tags: makeBusinessKit(name).tags, custom_fields: makeBusinessKit(name).crmFields, dashboards: makeBusinessKit(name).dashboards, academy: makeBusinessKit(name).academy, documentation: makeBusinessKit(name).documentation, settings: [], dependencies: [], post_install_steps: makeBusinessKit(name).checklist } })),
];
