...
export default function AIStoreCard({ card, onInstall, onDetails }: { card: AIStoreCardData; onInstall: (id: string) => void; onDetails: (card: AIStoreCardData) => void }) {
  ...
  return <article className="ai-store-card">
    ...
    <div className="ai-store-card-actions"><button type="button" className="ai-store-details-button" onClick={() => onDetails(card)}>Visualizar e aprender</button><button type="button" className="ai-store-install" onClick={() => onInstall(card.id)}>Instalar</button></div>
  </article>;
}
