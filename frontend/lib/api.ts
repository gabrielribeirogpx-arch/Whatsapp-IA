...
export async function resumeWhatsAppCampaign(campaignId: string) { const res = await apiFetch(`/api/whatsapp/campaigns/${campaignId}/resume`, { method:'POST' }); return parseApiResponse<WhatsAppCampaign>(res); }
export async function cancelWhatsAppCampaign(campaignId: string) { const res = await apiFetch(`/api/whatsapp/campaigns/${campaignId}/cancel`, { method:'POST' }); return parseApiResponse<WhatsAppCampaign>(res); }
...
export type CampaignAnalyticsSummary = { campaigns_created:number; campaigns_completed:number; total_recipients:number; total_sent:number; total_delivered:number; total_read:number; total_failed:number; delivery_rate:number|null; read_rate:number|null; failure_rate:number|null; timestamp_basis?: Record<string,string> };
export type CampaignAnalyticsRow = WhatsAppCampaign & { template_name?: string|null; template_category?: string|null; template_language?: string|null; delivery_rate?: number|null; read_rate?: number|null; failure_rate?: number|null; duration_seconds?: number|null };
export type CampaignAnalyticsPage = { items: CampaignAnalyticsRow[]; page:number; page_size:number; total:number };
export type TemplateAnalyticsRow = { template_id:string; template_name:string; category?:string|null; language?:string|null; campaigns:number; total_recipients:number; total_sent:number; total_delivered:number; total_read:number; total_failed:number; delivery_rate:number|null; read_rate:number|null };
export type TimelineAnalytics = { grain:string; items:Array<{ bucket:string; sent:number; delivered:number; read:number; failed:number }> };
export type FailureAnalyticsRow = { category:string; message:string; recommendation:string; count:number; percent:number; codes:string[] };
export type HeatmapAnalytics = { sufficient_data:boolean; items:Array<{ weekday:number; hour:number; count:number }> };
function campaignAnalyticsQuery(params: Record<string, string | number | undefined | null>) { const q = new URLSearchParams(); Object.entries(params).forEach(([k,v]) => { if (v !== undefined && v !== null && String(v) !== '') q.set(k, String(v)); }); const s = q.toString(); return s ? `?${s}` : ''; }
export async function getCampaignAnalyticsSummary(params: Record<string, string | number | undefined | null>): Promise<CampaignAnalyticsSummary> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/summary${campaignAnalyticsQuery(params)}`); return parseApiResponse<CampaignAnalyticsSummary>(res); }
export async function getCampaignAnalyticsTimeline(params: Record<string, string | number | undefined | null>): Promise<TimelineAnalytics> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/timeline${campaignAnalyticsQuery(params)}`); return parseApiResponse<TimelineAnalytics>(res); }
export async function getCampaignAnalyticsByCampaign(params: Record<string, string | number | undefined | null>): Promise<CampaignAnalyticsPage> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/by-campaign${campaignAnalyticsQuery(params)}`); return parseApiResponse<CampaignAnalyticsPage>(res); }
export async function getCampaignAnalyticsByTemplate(params: Record<string, string | number | undefined | null>): Promise<TemplateAnalyticsRow[]> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/by-template${campaignAnalyticsQuery(params)}`); return parseApiResponse<TemplateAnalyticsRow[]>(res); }
export async function getCampaignAnalyticsFailures(params: Record<string, string | number | undefined | null>): Promise<FailureAnalyticsRow[]> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/failures${campaignAnalyticsQuery(params)}`); return parseApiResponse<FailureAnalyticsRow[]>(res); }
export async function getCampaignAnalyticsHeatmap(params: Record<string, string | number | undefined | null>): Promise<HeatmapAnalytics> { const res = await apiFetch(`/api/whatsapp/campaigns/analytics/heatmap${campaignAnalyticsQuery(params)}`); return parseApiResponse<HeatmapAnalytics>(res); }
export function campaignAnalyticsExportUrl(params: Record<string, string | number | undefined | null>) { return `/api/whatsapp/campaigns/analytics/export${campaignAnalyticsQuery(params)}`; }
