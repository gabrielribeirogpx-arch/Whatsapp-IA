        ...
        <section className="login-premium-left relative hidden min-h-screen flex-col justify-between overflow-hidden bg-white px-8 py-7 lg:flex xl:px-14">
          ...
          <div className="relative z-10 animate-[fadeIn_700ms_ease-out_both]">
            ...
            <div className="login-hero-copy mt-8 max-w-xl">
              ...
              <h1 className="login-headline text-[2.65rem] font-black leading-[1.03] tracking-[-0.055em] text-[#0e1c2a] xl:text-5xl">
                Integre.<br />Automatize.<br />Escale <span className="text-[#55B521]">sem limites.</span>
              </h1>
              <p className="login-subtitle mt-4 max-w-lg text-base leading-7 text-slate-600">Acesse o poder do Wazza e conecte seus sistemas com segurança, inteligência e alta performance.</p>
            </div>
          </div>
        ...
        <section className="login-form-shell relative flex min-h-screen items-center justify-center px-4 py-5 sm:px-8 lg:px-10">
          <form className="login-premium-card w-full max-w-[540px] animate-[fadeInUp_650ms_ease-out_both] rounded-[26px] border border-white/80 bg-white/95 p-6 shadow-[0_24px_74px_rgba(15,23,42,0.13)] backdrop-blur-xl sm:p-7 lg:p-8" onSubmit={onSubmit}>
            <div className="login-card-header mb-4 text-center">
              <Image src="/Logo.svg" alt="Wazza" width={96} height={80} priority className="login-card-logo mx-auto h-[50px] w-[50px] object-contain drop-shadow-sm" />
            ...
            <div className="login-fields space-y-2">
              <label className="block" htmlFor="email">
                <span className="mb-1.5 block text-sm font-semibold text-slate-700">Email</span>
                <span className="login-input-wrap group flex h-[41px] items-center gap-2.5 rounded-2xl border border-slate-200 bg-white px-3 shadow-sm transition duration-300 focus-within:border-[#6CBF2A] focus-within:shadow-[0_0_0_4px_rgba(108,191,42,0.14)]">
            ...
            <div className="login-options mt-1.5 flex items-center justify-between gap-4 text-sm">
            ...
            <div className="login-captcha mt-1.5"><TurnstileWidget key={turnstileKey} action="login" token={turnstileToken} onToken={setTurnstileToken} onError={setError} /></div>

            <button type="submit" className="login-submit group mt-1.5 flex h-[43px] w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#55B521] to-[#43a114] px-4 text-base font-bold text-white shadow-[0_16px_34px_rgba(85,181,33,0.28)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_22px_44px_rgba(85,181,33,0.34)] active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60" disabled={isLoading || !turnstileToken}>
              {isLoading ? 'Entrando...' : 'Entrar'} <ArrowRight className="h-5 w-5 transition duration-300 group-hover:translate-x-1" />
            </button>

            <div className="login-divider my-2.5 flex items-center gap-4 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400"><span className="h-px flex-1 bg-slate-200" />OU<span className="h-px flex-1 bg-slate-200" /></div>

            <button type="button" className="login-google flex h-[43px] w-full items-center justify-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 text-base font-bold text-slate-700 shadow-sm transition duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-lg"><GoogleIcon />Continuar com Google</button>

            <div className="login-create mt-2 text-center">
              <p className="text-sm text-slate-500">Ainda não possui conta?</p>
              <Link href="/register" className="mt-1 flex h-[43px] w-full items-center justify-center gap-2 rounded-2xl border border-[#6CBF2A]/45 bg-white px-4 py-2 text-base font-bold text-[#389713] transition duration-300 hover:-translate-y-0.5 hover:border-[#55B521] hover:bg-[#6CBF2A]/5 hover:shadow-[0_16px_34px_rgba(85,181,33,0.14)]">Criar conta <ArrowRight className="h-5 w-5" /></Link>
            </div>
          </form>
        </section>
      </div>
