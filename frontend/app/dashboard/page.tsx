      ...
      <div className="grid w-full grid-cols-1 gap-4 xl:grid-cols-3">
        <div className={`${cardClassName} flex min-h-[232px] flex-col p-4 sm:p-5`}>
          <div className="mb-3 flex items-center justify-between">
            <p className="m-0 text-base font-semibold text-slate-900">Top fluxos</p>
            <button className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-600">Por conversas</button>
          </div>
          {flowsError ? <p className="text-sm text-red-700">{flowsError}</p> : viewModel.topFlows.length === 0 ? <div className="grid flex-1 place-items-center rounded-xl border border-dashed border-emerald-200 bg-gradient-to-b from-emerald-50/60 to-white px-4 text-center"><div className="space-y-2"><span className="mx-auto inline-flex h-9 w-9 items-center justify-center rounded-full border border-emerald-200 bg-white text-emerald-600 shadow-sm"><Zap size={16} /></span><p className="m-0 text-sm font-semibold text-slate-800">Nenhum fluxo ativo ainda</p><p className="m-0 text-xs leading-relaxed text-slate-500">Crie seu primeiro fluxo automatizado para começar a coletar métricas.</p></div></div> : <div className="space-y-3">{(viewModel.topFlows.slice(0,5)).map((flow) => {
