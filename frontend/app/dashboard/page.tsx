    ...
    return (
      ...
      <div className="grid w-full grid-cols-1 items-stretch gap-4 xl:grid-cols-3">
        ...
        <div className={`${cardClassName} flex min-h-[224px] flex-col p-4 sm:p-5`}>
          ...
          {viewModel.performance.avgResponseTimeSeconds === null && viewModel.performance.resolvedConversations === 0 && viewModel.performance.csat === null ? <div className="flex-1 rounded-lg bg-slate-50/60 px-3 py-3"><div className="mb-2.5 flex items-center gap-2"><span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-white text-slate-400 ring-1 ring-inset ring-slate-200"><TrendingUp size={14} strokeWidth={1.8} /></span><p className="m-0 text-xs font-medium leading-4 text-slate-600">Dados aparecerão aqui conforme as conversas acontecerem.</p></div><div className="grid grid-cols-2">{[1,2,3,4].map((item) => <div key={item} className={`px-3 py-2.5 ${item % 2 === 1 ? 'border-r border-slate-200/80' : ''} ${item <= 2 ? 'border-b border-slate-200/80' : ''}`}><SkeletonLine width="52%" height={13} /><div className="mt-2"><SkeletonLine width="42%" height={8} /></div></div>)}</div></div> : <div className="grid flex-1 grid-cols-2 rounded-lg bg-slate-50/50">{performanceKpis.map((item, index) => { const Icon = item.icon; return <div key={item.label} className={`flex min-h-[82px] items-center gap-3 px-3 py-3 ${index % 2 === 0 ? 'border-r border-slate-200/80' : ''} ${index < 2 ? 'border-b border-slate-200/80' : ''}`}><span className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md border ${item.className}`}><Icon size={14} strokeWidth={2} /></span><div className="min-w-0"><p className="m-0 text-lg font-semibold leading-tight tracking-[-0.02em] text-slate-950">{item.value}</p><p className="m-0 mt-1 text-[11px] font-medium leading-tight text-slate-500">{item.label}</p></div></div>; })}</div>}
