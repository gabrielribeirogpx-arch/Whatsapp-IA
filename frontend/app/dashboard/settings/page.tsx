...
Use uma variável da lista ou remova o marcador.`; return validateMetaVariables(mapped.bodyText); };
  ...
  return <section className='w-full min-w-0 px-4 py-6 sm:px-6 lg:px-8'>
    <div className='w-full min-w-0 space-y-5'>
      ...
      {tab === 'templates' && <TemplatesTab templates={templates} form={templateForm} setForm={setTemplateForm} error={templateError} loading={loading} onSubmit={(e: FormEvent) => run(async () => { e.preventDefault(); const msg = validateTemplate(); setTemplateError(msg); if (msg) throw new Error(msg); const mapped = friendlyToMeta(templateForm.friendly_body_text || templateForm.body_text || ''); await createTemplate({ ...templateForm, body_text: mapped.bodyText, body_raw_meta: mapped.bodyText, body_preview: renderExample(mapped.bodyText, mapped.variables), variables_json: mapped.variables }); setTemplateForm(baseTemplateForm); await refresh(); }, 'Template criado com sucesso', 'Falha ao criar template')} onSync={() => run(async () => { await syncTemplates(); await refresh(); }, 'Sincronização concluída', 'Erro ao sincronizar templates')} onSubmitTemplate={(id: string) => run(async () => { await submitTemplate(id); await refresh(); }, 'Template enviado para aprovação', 'Falha ao enviar template')} />}
