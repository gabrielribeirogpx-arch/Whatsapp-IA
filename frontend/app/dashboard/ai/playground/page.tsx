        ...
        <div className="fixed inset-0 z-50 bg-slate-900/30" onClick={() => setSelected(null)}>
          <aside className="ml-auto h-full w-full max-w-xl overflow-y-auto bg-white p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <section className="mt-6 space-y-4 text-sm">
              ...
              <div><h3 className="font-bold">Memórias recuperadas</h3><div className="mt-2 rounded-xl bg-slate-50 p-3 text-sm">Context Builder: {asText(selected.metadata.context_builder_enabled)} · Memórias: {asText(selected.metadata.long_term_memory_count)} · Tipos: {asText(selected.metadata.long_term_memory_types)} · Backend: {asText(selected.metadata.memory_backend)} · Latência: {asText(selected.metadata.memory_latency_ms)} ms</div></div>
