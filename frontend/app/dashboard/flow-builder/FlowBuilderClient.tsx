...
type MCPToolOption = { id: string; server_id?: string | null; tool_name?: string | null; display_name?: string | null; description?: string | null; input_schema?: Record<string, unknown> | null; is_enabled?: boolean; server_name?: string | null };
      ...
      <div className="flow-node-editor-content">
        ...
        {kind === 'ai_agent' && (
          <>
            ...
            <section id="agent-mcp-tools" className="flow-editor-tab-section"><h4>Ferramentas MCP</h4><p>Ferramentas descobertas via MCP ficam disponíveis ao ToolRegistry como <code>chamar_mcp</code>.</p><div className="flow-editor-subflow-header"><label className="flow-editor-radio"><input type="checkbox" checked={draft.allow_mcp_tools === true} onChange={(event) => onDraftChange({ allow_mcp_tools: event.target.checked, ...(event.target.checked ? { allowed_tools: Array.from(new Set([...allowedTools, 'chamar_mcp'])) } : {}) })} />Ativar ferramentas MCP</label><label className="flow-editor-field">Adicionar ferramenta<select value="" onChange={(event) => { addMcpTool(event.target.value); event.currentTarget.value = ''; }}><option value="">+ Adicionar ferramenta MCP</option>{activeMcpTools.filter((tool) => !mcpToolIds.includes(tool.id)).map((tool) => <option key={tool.id} value={tool.id}>[MCP] {toText(tool.display_name || tool.tool_name)} — {toText(tool.server_name) || 'Integração MCP'}</option>)}</select></label></div><label className="flow-editor-field">Limite de chamadas MCP<input type="number" min="0" max="3" value={toText(draft.max_mcp_calls || 3)} onChange={(event) => onDraftChange({ max_mcp_calls: Math.min(3, Math.max(0, Number(event.target.value || 3))) })} /></label><div className="flow-editor-subflow-list">{selectedMcpTools.map((tool) => <article key={tool.id} className="flow-editor-subflow-card"><div className="flow-editor-subflow-card-title"><strong>🔌 [MCP] {toText(tool.display_name || tool.tool_name)}</strong><button type="button" onClick={() => removeMcpTool(tool.id)}>Remover</button></div><small>{toText(tool.description) || 'Sem descrição.'}</small><small>Integração de origem: {toText(tool.server_name) || 'Integração MCP'}</small><code>{tool.id}</code></article>)}</div>{activeMcpTools.length === 0 ? <small className="flow-editor-muted">Nenhuma ferramenta MCP ativa encontrada neste tenant.</small> : null}</section><section id="agent-memory" className="flow-editor-tab-section"><h4>Memória</h4><p>Memória permite manter contexto entre mensagens.</p><label className="flow-editor-radio"><input type="checkbox" checked={draft.use_memory !== false} onChange={(event) => onDraftChange({ use_memory: event.target.checked })} />Usar memória da conversa</label><label className="flow-editor-radio flow-editor-disabled"><input type="checkbox" disabled />Usar memória de longo prazo</label><label className="flow-editor-radio flow-editor-disabled"><input type="checkbox" disabled />Permitir atualização automática da memória</label></section>
