    ...
    def execute(self, tool_type: str, tool_id: str, input: Any, context: ToolContext, config: dict[str, Any] | None = None) -> ToolResult:
        ...
        try:
            ...
            if not adapter.can_execute(str(tool_id), input, context, config or {}):
                ...
                return ToolResult(False, str(tool_type), tool_id=str(tool_id), error_code="tool_not_allowed", metadata=safe)
            ...
            return ToolResult(False, str(tool_type), tool_id=str(tool_id), error_code="budget_exceeded", error_message=str(exc), metadata=safe)
        except ToolRegistryError as exc:
            code = getattr(exc, "error_code", "tool_registry_error")
            logger.warning("event=tool_registry_error %s", {**safe, "error_code": code})
            return ToolResult(False, str(tool_type), tool_id=str(tool_id), error_code=code, error_message=str(exc), metadata=safe)
            ...
            return ToolResult(False, str(tool_type), tool_id=str(tool_id), error_code="tool_execution_error", error_message=type(exc).__name__, metadata=safe)
