...
class RuntimeV2DataCollectionExecutor(BaseNodeExecutor):
    ...
    def execute(self, db, *, snapshot, session, node, runtime_input):
        ...
        if not isinstance(waiting, dict):
            ...
            waiting = {'variable_name': data.get('variable_name'), 'data_type': data.get('data_type'), 'attempts': 0, 'max_attempts': max(1, int(data.get('max_attempts') or 1)), 'timeout_at': timeout_at.isoformat() if timeout_at else None, 'node_id': node_id, 'processed_message_ids': []}
            context.update({'waiting_for': 'data_collection', 'waiting_node_id': node_id, 'data_collection': waiting}); session.context = context
            actions = []
        ...
        if not result.valid:
            waiting['attempts'] = int(waiting.get('attempts') or 0) + 1; context['data_collection'] = waiting; session.context = context
            ...
            action = SendMessageAction(tenant_id=session.tenant_id, session_id=session.id, external_user_id=runtime_input.external_user_id, conversation_id=runtime_input.conversation_id, contact_id=runtime_input.contact_id, text=str(data.get('invalid_message') or 'Valor inválido. Tente novamente.'), metadata={'node_type': 'data_collection', 'attempt': waiting['attempts']})
            if waiting['attempts'] < int(waiting['max_attempts']): return NodeExecutionResult(actions=(action,), status='wait', next_node_id=node_id)
    ...
    def _finish(self, db, snapshot, session, context, node_id, data, handle, event):
        context.pop('waiting_for', None); context.pop('waiting_node_id', None); context.pop('data_collection', None); session.context = context
