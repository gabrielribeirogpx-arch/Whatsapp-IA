...
GRAPH_BASE_URL = os.getenv("META_GRAPH_BASE_URL", f"https://graph.facebook.com/{GRAPH_API_VERSION}").rstrip("/")
...
def create_meta_oauth_state(tenant_id: object, *, connection_type: ConnectionType = "cloud_api", nonce: str | None = None, issued_at: int | None = None, persist_nonce: bool = True) -> str:
    issued = issued_at or int(time.time())
    state_nonce = nonce or secrets.token_urlsafe(24)
    payload = {"tenant_id": str(tenant_id), "provider": "meta", "connection_type": connection_type, "nonce": state_nonce, "iat": issued}
...
def verify_meta_oauth_state(state: str, *, consume_nonce: bool = False) -> dict[str, Any]:
    try:
    ...
    if consume_nonce:
        ...
        if not expires_at or expires_at <= now:
            raise HTTPException(status_code=400, detail="Nonce Meta inválido ou reutilizado")
    return payload


def _redirect_uri() -> str:
    return os.getenv("META_REDIRECT_URI") or os.getenv("BACKEND_PUBLIC_URL", "").rstrip("/") + "/api/integrations/meta/callback"
...
def _connect_url(state: str) -> str:
    ...
    params = {
        ...
        "response_type": "code",
        "extras": json.dumps({"feature": "whatsapp_embedded_signup", "setup": {"solution": "coexistence"}}),
    }
...
def _provider_status(provider: TenantWhatsAppProvider | None) -> dict[str, Any]:
    ...
    return {
        ...
        "business_manager_id": getattr(provider, "business_manager_id", None) or (provider.business_id if provider else None),
        ...
        "display_name": provider.phone_display_name or provider.display_name if provider else None,
        "verified_name": provider.phone_verified_name or meta.get("verified_name") if provider else None,
        "phone_number": provider.business_phone_number_id or provider.phone_display_name or meta.get("display_phone_number") if provider else None,
    }
...
async def get_meta_connect_url(request: Request, db: Session = Depends(get_db), tenant: Tenant = Depends(get_current_tenant), connection_type: str | None = Query(default=None)):
    ...
    logger.info("META_COEX_CONNECT_URL_CREATED tenant_id=%s connection_type=%s state_present=%s", tenant.id, normalized, bool(state))
    return {"url": url, "state": state, "connection_type": normalized}


def _meta_get(path: str, token: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    response = requests.get(f"{GRAPH_BASE_URL}/{path.lstrip('/')}", headers={"Authorization": f"Bearer {token}"}, params=params or {}, timeout=20)
...
def _exchange_code_for_token(code: str, redirect_uri: str) -> str:
    response = requests.get(
        ...
        params={"client_id": os.getenv("META_APP_ID") or os.getenv("FACEBOOK_APP_ID"), "client_secret": os.getenv("META_APP_SECRET") or os.getenv("FACEBOOK_APP_SECRET"), "redirect_uri": redirect_uri, "code": code},
        timeout=20,
    )
    if response.status_code >= 400:
        raise HTTPException(status_code=400, detail="Falha ao trocar code Meta por token")
...
def _discover_meta_business(token: str) -> dict[str, Any]:
    businesses = _meta_get("me/businesses", token, {"fields": "id,name"}).get("data", [])
    ...
    if business_id:
        for edge in ("owned_whatsapp_business_accounts", "client_whatsapp_business_accounts"):
            wabas.extend(_meta_get(f"{business_id}/{edge}", token, {"fields": "id,name"}).get("data", []))
    if not wabas:
        wabas = _meta_get("me/whatsapp_business_accounts", token, {"fields": "id,name"}).get("data", [])
    waba = _first(wabas) or {}
    waba_id = waba.get("id")
    if not waba_id:
        raise HTTPException(status_code=400, detail="Nenhuma WABA encontrada para a conta Meta")
    phones = _meta_get(f"{waba_id}/phone_numbers", token, {"fields": "id,display_phone_number,verified_name,quality_rating,name_status,code_verification_status,platform_type"}).get("data", [])
    phone = _first(phones) or {}
    phone_id = phone.get("id")
    if not phone_id:
        raise HTTPException(status_code=400, detail="Nenhum número WhatsApp encontrado na WABA")
    logger.info("META_COEX_PHONE_DISCOVERED phone_found=True")
    return {"business_id": business_id, "business_name": business.get("name"), "waba_id": waba_id, "waba_name": waba.get("name"), "phone": phone}


@router.get("/callback")
def meta_callback(request: Request, code: str | None = None, state: str | None = None, db: Session = Depends(get_db)):
    logger.info("META_COEX_CALLBACK_RECEIVED has_code=%s has_state=%s", bool(code), bool(state))
    ...
    existing = db.execute(select(TenantWhatsAppProvider).where(TenantWhatsAppProvider.tenant_id == tenant_id, TenantWhatsAppProvider.provider_type == "meta_cloud").order_by(TenantWhatsAppProvider.is_active.desc(), TenantWhatsAppProvider.updated_at.desc())).scalars().first()
    provider = existing or TenantWhatsAppProvider(tenant_id=tenant_id, provider_type="meta_cloud")
    ...
    provider.business_phone_number_id = phone.get("display_phone_number") or phone_number_id
    ...
    provider.connection_type = "cloud_api_coexistence" if connection_type == "cloud_api_coexistence" else connection_type
    ...
    provider.onboarding_metadata = {"provider": "meta", "embedded_signup": True, "business_id": discovered.get("business_id")}
    provider.metadata_json = {"onboarding_source": "meta_embedded_signup", "business_name": discovered.get("business_name"), "waba_name": discovered.get("waba_name"), "display_phone_number": phone.get("display_phone_number"), "verified_name": phone.get("verified_name"), "quality_rating": phone.get("quality_rating"), "name_status": phone.get("name_status"), "code_verification_status": phone.get("code_verification_status"), "platform_type": phone.get("platform_type")}
    provider.status = "connected"
    provider.connection_status = "connected"
    provider.is_active = True
    db.execute(update(TenantWhatsAppProvider).where(TenantWhatsAppProvider.tenant_id == tenant_id, TenantWhatsAppProvider.id != provider.id).values(is_active=False)) if existing else db.execute(update(TenantWhatsAppProvider).where(TenantWhatsAppProvider.tenant_id == tenant_id).values(is_active=False))
    if not existing:
        db.add(provider)
    db.commit()
    logger.info("META_COEX_CONNECTION_UPDATED tenant_id=%s provider_id=%s", tenant_id, provider.id) if existing else logger.info("META_COEX_CONNECTION_CREATED tenant_id=%s provider_id=%s", tenant_id, provider.id)
