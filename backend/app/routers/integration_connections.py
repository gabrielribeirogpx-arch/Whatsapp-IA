...
def disconnect_connection(provider: str, request: Request, tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db), user: TenantUser = Depends(get_current_user)):
    ...
    existing = service.get_connection(tenant.id, normalized_provider)
    ...
    db.flush()
    write_audit_log(db, action="integration_disconnected", tenant_id=tenant.id, user_id=user.id, entity_type="integration", entity_id=getattr(connection, "id", normalized_provider), metadata={"provider": normalized_provider, "old_status": old_status, "new_status": "disconnected"}, request=request)
    db.commit()
    return service.to_public_status(connection, provider=normalized_provider)
