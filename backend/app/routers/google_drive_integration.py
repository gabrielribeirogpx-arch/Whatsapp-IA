...
def get_google_drive_connect_tenant(
    ...
    logger.info(
        "event=google_drive_connect_tenant_resolved tenant_id=%s source=%s",
...
def _state_secret() -> bytes:
    secret = (
        os.getenv("GOOGLE_DRIVE_STATE_SECRET")
...
def _redirect_uri(request: Request) -> str:
    try:
        return oauth_callback_url(request, "/api/integrations/google-drive/callback", "GOOGLE_DRIVE_REDIRECT_URI")
    except ValueError as exc:
        raise HTTPException(status_code=500, detail="GOOGLE_DRIVE_REDIRECT_URI inválida") from exc


def _validate_google_drive_connect_config(request: Request) -> None:
    if not _client_id():
        raise HTTPException(status_code=500, detail="GOOGLE_DRIVE_CLIENT_ID não configurado")
    if not _client_secret():
        raise HTTPException(status_code=500, detail="GOOGLE_DRIVE_CLIENT_SECRET não configurado")
    _state_secret()
...
def connect_google_drive(
    ...
    try:
        logger.warning("ENTERED GOOGLE_DRIVE CONNECT ENDPOINT")
        redirect_uri = _redirect_uri(request)
        callback_path = urlparse(redirect_uri).path
        frontend_return_url = _frontend_oauth_result_url("connected")
        logger.info(
            "GOOGLE_DRIVE_OAUTH_CONNECT_URL_REQUESTED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s tenant_slug=%s tenant_id=%s x_tenant_slug=%s x_tenant_id=%s x_tenant_id_upper=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        _validate_google_drive_connect_config(request)
        ...
        logger.info(
            "GOOGLE_DRIVE_TENANT_RESOLUTION_RESULT resolved=%s tenant_id=%s source=%s",
            bool(resolution),
        ...
        logger.info(
            "GOOGLE_DRIVE_OAUTH_CONNECT_URL_GENERATED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        logger.exception(
            "GOOGLE_DRIVE_CONNECT_EXCEPTION exception_type=%s exception_message=%s",
            type(exc).__name__,
            str(exc),
        )
        raise
...
@router.get("/callback", name="google_drive_callback")
def google_drive_callback(request: Request, code: str | None = None, state: str | None = None, error: str | None = None, db: Session = Depends(get_db)):
    try:
        redirect_uri = _redirect_uri(request)
        callback_path = urlparse(str(request.url)).path
        frontend_return_url = _frontend_oauth_result_url("connected")
        logger.info(
            "GOOGLE_DRIVE_OAUTH_CALLBACK_RECEIVED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s has_code=%s has_state=%s error=%s integration=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        logger.info(
            "GOOGLE_DRIVE_OAUTH_CALLBACK_STATE_DECODED provider=%s state_provider=%s tenant_id=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s integration=%s",
            PROVIDER,
        ...
        db.rollback()
        logger.exception(
            "GOOGLE_DRIVE_CALLBACK_EXCEPTION exception_type=%s exception_message=%s",
            type(exc).__name__,
            str(exc),
        )
...
def google_drive_disconnect(request: Request, tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db), user: TenantUser = Depends(get_current_user)):
