...
def _frontend_base_url() -> str:
    ...
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        logger.warning("GOOGLE_SHEETS_INVALID_FRONTEND_URL value=%s", configured)
...
def get_google_sheets_connect_tenant(
    ...
    logger.info(
        "event=google_sheets_connect_tenant_resolved tenant_id=%s source=%s",
...
def _client_id() -> str:
    return (
        os.getenv("GOOGLE_SHEETS_CLIENT_ID")
...
def _client_secret() -> str:
    return (
        os.getenv("GOOGLE_SHEETS_CLIENT_SECRET")
...
def _state_secret() -> bytes:
    secret = (
        os.getenv("GOOGLE_SHEETS_STATE_SECRET")
...
def _redirect_uri(request: Request) -> str:
    configured = (os.getenv("GOOGLE_SHEETS_REDIRECT_URI") or "").strip()
    redirect_uri = configured or f"{_backend_base_url(request)}/api/integrations/google-sheets/callback"
    parsed = urlparse(redirect_uri)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise HTTPException(status_code=500, detail="GOOGLE_SHEETS_REDIRECT_URI inválida")
    logger.info("GOOGLE_SHEETS_REDIRECT_URI_RESOLVED redirect_uri=%s source=%s", redirect_uri, "env" if configured else "fallback")
    return redirect_uri


def _validate_google_sheets_connect_config(request: Request) -> None:
    if not _client_id():
        raise HTTPException(status_code=500, detail="GOOGLE_SHEETS_CLIENT_ID não configurado")
    if not _client_secret():
        raise HTTPException(status_code=500, detail="GOOGLE_SHEETS_CLIENT_SECRET não configurado")
    _state_secret()
...
def connect_google_sheets(
    ...
    try:
        logger.warning("ENTERED GOOGLE_SHEETS CONNECT ENDPOINT")
        redirect_uri = _redirect_uri(request)
        callback_path = urlparse(redirect_uri).path
        frontend_return_url = _frontend_oauth_result_url("connected")
        logger.info(
            "GOOGLE_SHEETS_OAUTH_CONNECT_URL_REQUESTED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s tenant_slug=%s tenant_id=%s x_tenant_slug=%s x_tenant_id=%s x_tenant_id_upper=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        _validate_google_sheets_connect_config(request)
        ...
        logger.info(
            "GOOGLE_SHEETS_TENANT_RESOLUTION_RESULT resolved=%s tenant_id=%s source=%s",
            bool(resolution),
        ...
        logger.info(
            "GOOGLE_SHEETS_OAUTH_CONNECT_URL_GENERATED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        logger.exception(
            "GOOGLE_SHEETS_CONNECT_EXCEPTION exception_type=%s exception_message=%s",
            type(exc).__name__,
            str(exc),
        )
        raise
...
@router.get("/callback", name="google_sheets_callback")
def google_sheets_callback(request: Request, code: str | None = None, state: str | None = None, error: str | None = None, db: Session = Depends(get_db)):
    try:
        redirect_uri = _redirect_uri(request)
        callback_path = urlparse(str(request.url)).path
        frontend_return_url = _frontend_oauth_result_url("connected")
        logger.info(
            "GOOGLE_SHEETS_OAUTH_CALLBACK_RECEIVED provider=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s has_code=%s has_state=%s error=%s integration=%s",
            PROVIDER,
            redirect_uri,
            SCOPES,
            callback_path,
        ...
        logger.info(
            "GOOGLE_SHEETS_OAUTH_CALLBACK_STATE_DECODED provider=%s state_provider=%s tenant_id=%s redirect_uri=%s scopes=%s callback_path=%s frontend_return_url=%s integration=%s",
            PROVIDER,
        ...
        logger.exception(
            "GOOGLE_SHEETS_CALLBACK_EXCEPTION exception_type=%s exception_message=%s",
            type(exc).__name__,
            str(exc),
        )
...
def google_sheets_status(tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db)):
...
def google_sheets_disconnect(tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db)):
    service = IntegrationConnectionService(db)
