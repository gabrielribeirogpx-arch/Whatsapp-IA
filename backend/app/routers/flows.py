...
def _normalize_flow_handle(value: Any) -> str:
    return str(value or "").strip().lower()


def _builder_flow_handle_id(value: Any, fallback: str) -> str:
    normalized = re.sub(r"[^a-z0-9_]", "", re.sub(r"\s+", "_", str(value or "").strip().lower()))
...
def _flow_handle_exists(available_handles: list[str], handle: str | None) -> bool:
    normalized = _normalize_flow_handle(handle)
    return normalized in {_normalize_flow_handle(item) for item in available_handles}
...
def validate_flow_payload_or_400(
    ...
    for index, edge in enumerate(edges or []):
        ...
        if target not in node_ids:
            invalid_reasons.append("target_node_not_found")
        if source in node_ids and not _flow_handle_exists(node_handle_map[source]["source"], source_handle):
            invalid_reasons.append("source_handle_not_found")
        if target in node_ids and not _flow_handle_exists(node_handle_map[target]["target"], target_handle):
            invalid_reasons.append("target_handle_not_found")
        if invalid_reasons:
            detail = {
                "code": "VALIDATION_ERROR",
                "error": "EDGE_REFERENCE_NOT_FOUND",
                "message": "Edge references a missing node or handle.",
                "edge_index": index,
                "edge_id": edge.get("id"),
...
def get_tenant_flow_analytics(
    ...
    requested_period = range or period
    resolved_period = resolve_analytics_period(requested_period)
    if resolved_period != requested_period:
        logger.info("[FLOW ANALYTICS] invalid period=%s fallback=%s allowed=%s", requested_period, resolved_period, "|".join(PERIODS.keys()))

    selected_version_id = None
    all_versions = True
    if version == "active":
        selected_version_id = flow.published_version_id or flow.current_version_id
        all_versions = selected_version_id is None
    elif version and version != "all":
        try:
            selected_version_id = uuid.UUID(version)
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid flow version")
        exists = db.query(FlowVersion.id).filter(FlowVersion.id == selected_version_id, FlowVersion.flow_id == flow.id, FlowVersion.tenant_id == tenant_uuid).first()
        if not exists:
            raise HTTPException(status_code=404, detail="Flow version not found")
        all_versions = False

    analytics = get_flow_analytics(db=db, tenant_id=tenant_uuid, flow_id=flow.id, period=resolved_period, flow_version_id=selected_version_id, all_versions=all_versions)
