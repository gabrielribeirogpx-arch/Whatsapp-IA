...
from app.schemas.whatsapp_business import (
    TenantWhatsAppProviderCreate,
    TenantWhatsAppProviderOut,
    TenantWhatsAppProviderUpdate,
)
...
def create_provider(
    request: Request,
    payload: TenantWhatsAppProviderCreate,
    db: Session = Depends(get_db),
    tenant: Tenant = Depends(get_current_tenant),
    ...
    try:
        ...
        write_audit_log(
            ...
            tenant_id=tenant.id,
            user_id=user.id,
            entity_type="whatsapp_provider",
            entity_id=provider.id,
            metadata={"operation": "create", "provider_type": provider.provider_type},
            request=request,
            commit=True,
        )
        return provider
        ...
        raise HTTPException(
            status_code=500,
            detail="Erro ao criar provider. Verifique a configuração e tente novamente.",
        ) from exc
...
def patch_provider(
    provider_id: str,
    payload: TenantWhatsAppProviderUpdate,
    request: Request,
    db: Session = Depends(get_db),
    tenant: Tenant = Depends(get_current_tenant),
    user: TenantUser = Depends(get_current_user),
    ...
    try:
        ...
        action = (
            "API_KEY_UPDATED"
            if any(field in {"api_key", "access_token"} for field in fields)
            else "WHATSAPP_PROVIDER_UPDATED"
        )
        write_audit_log(
            db,
            action=action,
            ...
            entity_type="whatsapp_provider",
            entity_id=provider.id,
            metadata={"operation": "update", "fields": sorted(fields)},
            request=request,
            commit=True,
        )
        return provider
...
def test_worker_provider_token(
    ...
    return whatsapp_provider_service.test_worker_active_provider_connection(
        db, tenant.id
    )


@router.post("/diagnostics/runtime-send")
def runtime_send_diagnostics(
    db: Session = Depends(get_db), tenant: Tenant = Depends(get_current_tenant)
):
...
def test_provider(
    ...
    try:
        return whatsapp_provider_service.test_provider_connection(
            db, tenant.id, provider_id
        )
    except ValueError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
