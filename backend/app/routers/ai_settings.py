...
def _to_out(tenant_id, setting: TenantAISetting | None) -> TenantAISettingsOut:
    if not setting:
        return TenantAISettingsOut(tenant_id=tenant_id, provider="wazza_default", model=None, has_api_key=False)
...
def update_ai_settings(payload: TenantAISettingsUpdate, tenant: Tenant = Depends(get_current_tenant), db: Session = Depends(get_db)):
    ...
    setting.model = payload.model.strip() if payload.model else None
    ...
    setting.embedding_model = payload.embedding_model.strip() if payload.embedding_model else None
